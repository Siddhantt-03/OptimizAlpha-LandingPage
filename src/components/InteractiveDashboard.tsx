...
function MetricCard({ label, value, change, isPositive, benchmark, compact = false }: MetricCardProps) {
  return (
        ...
        <div className="h-16 border-b border-tealmint/10 px-6 flex items-center justify-between shrink-0">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs font-mono text-tealmint">
              ...
              {activeTab === 'pe' && (isHero ? 'Vintage PE' : 'Private Equity Vintage')}
            </span>
          </div>
        ...
        <div className={`flex-1 overflow-y-auto p-5 flex flex-col gap-5 scroll-hide-scrollbar ${
            ...
            <>
              ...
              {isHero ? (
                <div className="grid grid-cols-3 gap-3">
                  <div className="bg-gradient-to-br from-ocean/20 to-navy border border-tealmint/20 rounded-xl p-3.5 flex flex-col justify-between shadow-md">
                    <span className="text-[9px] font-mono text-tealmint uppercase tracking-wider block">Portfolio AUM</span>
                    <div className="text-lg md:text-xl font-mono font-bold text-pearl mt-1.5">$257.8M</div>
                    <div className="text-[8px] text-pearl/40 mt-1 flex items-center gap-1">
                      <span className="w-1.5 h-1.5 rounded-full bg-tealmint animate-pulse" />
                      <span>Live</span>
                    </div>
                  </div>
                  <MetricCard label="YTD Return" value="+8.31%" change="8.3%" isPositive={true} compact={true} />
                  <MetricCard label="Volatility" value="11.42%" change="1.2%" isPositive={false} compact={true} />
                </div>
              ) : (
              ...
              {/* Two columns charts */}
              {isHero ? (
                /* Hero Single Premium Performance Chart */
                <div className="bg-[#0b141e] border border-tealmint/10 rounded-xl p-4 flex flex-col justify-between shadow-md">
                  <div className="flex justify-between items-center mb-3">
                    <div>
                      <h4 className="text-xs font-semibold text-pearl">Portfolio Performance</h4>
                      <span className="text-[9px] font-mono text-pearl/40">TWR vs Benchmark (OBS_TGT)</span>
                    </div>
                    <div className="flex bg-navy border border-tealmint/15 rounded-lg p-0.5 text-[9px] font-mono shrink-0">
                      <button
                        onClick={() => setTimeframe('cumulative')}
                        className={`px-1.5 py-0.5 rounded text-[8px] transition-all ${timeframe === 'cumulative' ? 'bg-ocean text-pearl font-semibold' : 'text-pearl/60'}`}
                      >
                        Cum
                      </button>
                      <button
                        onClick={() => setTimeframe('annualized')}
                        className={`px-1.5 py-0.5 rounded text-[8px] transition-all ${timeframe === 'annualized' ? 'bg-ocean text-pearl font-semibold' : 'text-pearl/60'}`}
                      >
                        Ann
                      </button>
                      <button
                        onClick={() => setTimeframe('ytd')}
                        className={`px-1.5 py-0.5 rounded text-[8px] transition-all ${timeframe === 'ytd' ? 'bg-ocean text-pearl font-semibold' : 'text-pearl/60'}`}
                      >
                        YTD
                      </button>
                    </div>
                  ...
                  {/* SVG Chart */}
                  <div className="relative h-36 w-full border-b border-l border-pearl/10 mt-1">
                    <svg className="w-full h-full" viewBox="0 0 720 200" preserveAspectRatio="none">
                      {/* Grid Lines */}
                      <line x1="0" y1="50" x2="720" y2="50" stroke="rgba(218,255,251,0.04)" strokeDasharray="4" />
                      <line x1="0" y1="100" x2="720" y2="100" stroke="rgba(218,255,251,0.04)" strokeDasharray="4" />
                      <line x1="0" y1="150" x2="720" y2="150" stroke="rgba(218,255,251,0.04)" strokeDasharray="4" />

                      {/* Area gradient for portfolio */}
                      <defs>
                        <linearGradient id="chartGradientHero" x1="0" y1="0" x2="0" y2="1">
                          <stop offset="0%" stopColor="#64CCC5" stopOpacity="0.2" />
                          <stop offset="100%" stopColor="#176B87" stopOpacity="0.0" />
                        </linearGradient>
                      </defs>

                      {/* Filled Area */}
                      <path d={`${linePoints} L 680,200 L 40,200 Z`} fill="url(#chartGradientHero)" />
                      ...
                      <circle cx="520" cy="70" r="4.5" fill="#64CCC5" stroke="#001C30" strokeWidth="2" />
                    </svg>

                    {/* Hover Stats Tag */}
                    <div className="absolute top-3 left-[60%] bg-ocean/95 border border-tealmint/30 rounded px-1.5 py-0.5 text-[8px] font-mono shadow-md backdrop-blur-sm">
                      <span className="font-bold text-tealmint">Portfolio: +8.31%</span>
                    </div>
                  </div>

                  <div className="flex justify-between items-center text-[8px] font-mono text-pearl/40 mt-2 px-1">
                    <span>JAN</span>
                    <span>MAR</span>
                    <span>MAY</span>
                    <span>JUL</span>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                  ...
                  <div className="lg:col-span-2 bg-[#0b141e] border border-tealmint/10 rounded-xl p-5 flex flex-col justify-between shadow-md">
              ...
              {!isHero && (
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                  ...
                  <div className="bg-[#0b141e] border border-tealmint/10 rounded-xl p-5 shadow-md">
          ...
          {activeTab === 'assets' && (
            <>
              ...
              <div className={`grid gap-4 ${isHero ? 'grid-cols-2' : 'grid-cols-2 md:grid-cols-4'}`}>
                <div className="bg-[#0b141e] border border-tealmint/10 rounded-xl p-3.5 md:p-4 shadow-md">
                  <span className="text-[10px] font-mono text-pearl/40 block">EQUITY</span>
                  <span className="text-lg md:text-xl font-mono font-bold text-pearl block mt-1">$70.4M</span>
                ...
                <div className="bg-[#0b141e] border border-tealmint/10 rounded-xl p-3.5 md:p-4 shadow-md">
                  ...
                  <span className="text-lg md:text-xl font-mono font-bold text-pearl block mt-1">$163.8M</span>
                  <div className="w-full bg-navy h-1.5 rounded-full mt-3 overflow-hidden">
                ...
                {!isHero && (
                  <>
                    <div className="bg-[#0b141e] border border-tealmint/10 rounded-xl p-4 shadow-md">
              ...
              <div className={`grid gap-6 ${isHero ? 'grid-cols-1' : 'grid-cols-1 lg:grid-cols-2'}`}>
                {/* Bar chart exposure */}
                <div className="bg-[#0b141e] border border-tealmint/10 rounded-xl p-4 md:p-5 flex flex-col justify-between shadow-md">
                  <div>
                  ...
                  <div className="h-32 md:h-44 w-full mt-6 relative flex items-end justify-around border-b border-pearl/10 pb-2">
                    {/* 2024 bars */}
                    <div className="flex flex-col items-center gap-1.5">
                      <div className="flex items-end gap-1.5 h-24 md:h-32">
                        <div className="w-3 md:w-4 bg-tealmint/60 h-16 md:h-20 rounded-t" />
                        <div className="w-3 md:w-4 bg-ocean/60 h-22 md:h-28 rounded-t" />
                        <div className="w-3 md:w-4 bg-pearl/60 h-6 md:h-10 rounded-t" />
                      </div>
                      <span className="text-[8px] md:text-[9px] font-mono text-pearl/50">2024</span>
                    </div>

                    {/* 2025 bars */}
                    <div className="flex flex-col items-center gap-1.5">
                      <div className="flex items-end gap-1.5 h-24 md:h-32">
                        <div className="w-3 md:w-4 bg-tealmint h-20 md:h-24 rounded-t" />
                        <div className="w-3 md:w-4 bg-ocean h-24 md:h-32 rounded-t" />
                        <div className="w-3 md:w-4 bg-pearl h-8 md:h-12 rounded-t" />
                      </div>
                      <span className="text-[8px] md:text-[9px] font-mono text-pearl/50">2025</span>
                    </div>
                  </div>

                  <div className="flex justify-center gap-4 text-[8px] md:text-[9px] font-mono text-pearl/60 mt-3">
                    <span className="flex items-center gap-1">
                      <span className="w-2 md:w-2.5 h-1.5 md:h-2 rounded-sm bg-tealmint inline-block" />
                      <span>Equity</span>
                    </span>
                    <span className="flex items-center gap-1">
                      <span className="w-2 md:w-2.5 h-1.5 md:h-2 rounded-sm bg-ocean inline-block" />
          ...
          {activeTab === 'pe' && (
            <>
              {/* Vintage PE Metrics */}
              <div className={`grid gap-6 ${isHero ? 'grid-cols-1' : 'grid-cols-1 lg:grid-cols-3'}`}>
                <div className={`${isHero ? '' : 'lg:col-span-2'} bg-[#0b141e] border border-tealmint/10 rounded-xl p-4 md:p-5 shadow-md`}>
                  <h4 className="text-xs md:text-sm font-semibold text-pearl mb-3 md:mb-4">Vintage PE Performance Summary</h4>
                  <div className="overflow-x-auto">
                    <table className="w-full text-left font-mono text-[10px] md:text-xs">
                      <thead>
                        <tr className="border-b border-pearl/10 text-pearl/40 uppercase text-[8px] md:text-[9px] tracking-wider">
                          <th className="py-2.5">Vintage</th>
                          <th className="py-2.5">Commitment</th>
                          {!isHero && <th className="py-2.5">DPI (Realized)</th>}
                      ...
                      <tbody>
                        <tr className="border-b border-pearl/5">
                          <td className="py-2.5 font-semibold text-pearl">Fund 2018</td>
                          <td className="py-2.5">$25.0M</td>
                          {!isHero && <td className="py-3 text-tealmint">0.82x</td>}
                          <td className="py-2.5">1.64x</td>
                          <td className="py-2.5 text-tealmint font-semibold">18.42%</td>
                        </tr>
                        <tr className="border-b border-pearl/5">
                          <td className="py-2.5 font-semibold text-pearl">Fund 2020</td>
                          <td className="py-2.5">$30.0M</td>
                          {!isHero && <td className="py-3 text-tealmint">0.34x</td>}
                          <td className="py-2.5">1.42x</td>
                          <td className="py-2.5 text-tealmint font-semibold">14.10%</td>
                        </tr>
                        {!isHero && (
                          <tr className="border-b border-pearl/5">
                            <td className="py-3 font-semibold text-pearl">Fund 2022</td>
